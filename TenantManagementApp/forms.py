...
from TenantManagementApp.models import TblTenant, TblAgent
...
class TenantRegistratonForm(ModelForm):
    class Meta:
        model = TblTenant
        exclude = ('tn_agent','tn_is_active','tn_status','tn_joining_date')
        
    tn_name = forms.CharField( help_text="Tenant's Name", max_length=25,required=False)
    ...
    tn_profile = forms.ImageField(
        help_text='Upload Tenant\'s Picture here:', max_length=200, required=True)
